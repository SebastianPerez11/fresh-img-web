...
const $$ItemImg = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<a class="overflow-hidden rounded-lg min-w-[350px]"${addAttribute(`/${id}`, "href")}> <img${addAttribute(`view-transition-name: image-${id};`, "style")} class="w-full object-cover aspect-video rounded-lg transition-transform hover:scale-110"${addAttribute(img, "src")}${addAttribute(title, "alt")}> </a> <!-- min-sm:max-w-72 max-2xl:max-w-[445px]  -->`;
